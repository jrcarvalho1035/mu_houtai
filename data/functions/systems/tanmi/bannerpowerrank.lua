module("bannerpowerrank", package.seeall)

local rankingListName      = "bannerrpowerank"
local rankingListFile      = "bannerrpowerank.rank"
...
local function updateDynamicFirstCache(actor_id)
	...
	if #rankTbl ~= 0 then 
		...
		if actor_id == nil or actor_id == Ranking.getId(prank) then  
			worship.updateDynamicFirstCache(Ranking.getId(prank), RankingType_BannerPower)
		end
	end
end
...
function updateRankingList(actor, power)
    ...
    Ranking.setSubInt(item, 1, LActor.getVipLevel(actor))
...
function s2cRankingList(actor)
	...
	LDataPack.writeShort(npack, RankingType_BannerPower)
...
_G.onReqBannerPowerRanking = onReqRanking
