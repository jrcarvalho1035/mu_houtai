module("enhancepowerrank", package.seeall)


------------------------------------------------强化战力排行榜内容--------------------------------------
local rankingListName      = "enhancepowerrank"
local rankingListFile      = "enhancepowerrank.rank"
...
local function updateDynamicFirstCache(actor_id)
	...
	if #rankTbl ~= 0 then 
		...
		if actor_id == nil or actor_id == Ranking.getId(prank) then  
			worship.updateDynamicFirstCache(Ranking.getId(prank), RankingType_EnhancePower)
		end
	end
end
...
function s2cRankingList(actor)
	...
	LDataPack.writeShort(npack, RankingType_EnhancePower)
...
_G.onReqEnhancePowerRanking = onReqRanking
----------------------------------------------------------------------------------------------------------------------------
