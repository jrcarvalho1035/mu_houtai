--七天限购
module("subactivity2", package.seeall)
...
local function onGetReward(actor, config, id, index, record)
	...
	if ret then
		...
		actoritem.reduceItem(actor, config[index].currencyType, config[index].price, "activity type2 buy")
		actoritem.addItems(actor, config[index].rewards, "activity type2 rewards")
	end
