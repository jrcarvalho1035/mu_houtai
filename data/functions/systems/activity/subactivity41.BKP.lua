...
local function getActorVar(actor, id)
	...
	if ActivityType41Config[id][1].subType == minType.diamond then
...
function getConditionPer(actor, id, config)
    ...
    if subType == minType.diamond then
        local var = getActorVar(actor, id)
        return var.diamond
    end
...
local function checkLevelReward(actor, config, id, idx, var)
	...
	local cond = getConditionPer(actor, id, config[idx])
    if cond < config[idx].condition then
        return false
    end
	if var.rewardsRecord == nil then
        var.rewardsRecord = 0
    end
	if not actoritem.checkEquipBagSpaceJob(actor, config[idx].rewards) then
        return false
    end
    ...
    local rewards = {}
    --Adicione a recompensa de sua escolha à lista de recompensas
	local chooses = var.chooses[idx] or {}
	...
	--Adicione recompensas fixas à lista de recompensas
