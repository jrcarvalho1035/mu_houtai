...
function addAnger(anger, actorid, kalimaBossLv, bossId)
	...
	-- 	local lv, zhuansheng = zhuansheng.getZhuanSheng(kalimaBossLv)
...
function c2sQuintonFight(actor, packet)
	...
	local data = getGlobalData()
	if data.isOpen ~= 1 then return end
	local conf = QuaintonFubenConfig[1]
...
function s2cQuintonReward(isWin, config, aid, name, rank, extraReward, reward, ins, damage)
	...
	if actor and actoritem.checkEquipBagSpaceJob(actor, items) then --在线或背包够位
...
--跨服协议
---------------------------------------------------------------------------------
function sendQuaintonFbInfo(serverId)
    if not System.isBattleSrv() then return end
    local bossData = getBossData()
    if bossData.hfuben == nil then return end
    local pack = LDataPack.allocPacket()
    LDataPack.writeByte(pack, CrossSrvCmd.SCKalimaCmd)
    LDataPack.writeByte(pack, CrossSrvSubCmd.SCKalimaCmd_QuaintonfbInfo)
    LDataPack.writeInt64(pack, bossData.hfuben)
    System.sendPacketToAllGameClient(pack, serverId or 0)
end

function onQuaintonFbInfo(sId, sType, dp)
    if System.isCrossWarSrv() then return end
    local bossData = getBossData()
    bossData.hfuben =LDataPack.readInt64(dp)
end

function sendQuaintonInfo(serverId)
    if not System.isBattleSrv() then return end
    local data = getGlobalData()
    local pack = LDataPack.allocPacket()
    LDataPack.writeByte(pack, CrossSrvCmd.SCKalimaCmd)
    LDataPack.writeByte(pack, CrossSrvSubCmd.SCKalimaCmd_QuaintonInfo)
    LDataPack.writeShort(pack, data.bossLv)
    LDataPack.writeInt(pack, data.appearTime)
    LDataPack.writeChar(pack, data.isOpen)
    LDataPack.writeInt(pack, data.anger)
    System.sendPacketToAllGameClient(pack, serverId or 0)
end

function onQuaintonInfo(sId, sType, dp)
    if System.isCrossWarSrv() then return end
    local data = getGlobalData()
    data.bossLv = LDataPack.readShort(dp)
    data.appearTime = LDataPack.readInt(dp)
    data.isOpen = LDataPack.readChar(dp)
    data.anger = LDataPack.readInt(dp)
    notifyQuintonInfo()
    s2cQuintonAppear()
end

function OnQuaintonConnected(serverId, serverType)
    if not System.isBattleSrv() then return end
    sendQuaintonFbInfo(serverId)
    sendQuaintonInfo(serverId)
end
...
local function fuBenInit()
	...
	csbase.RegConnected(OnQuaintonConnected)

	csmsgdispatcher.Reg(CrossSrvCmd.SCKalimaCmd, CrossSrvSubCmd.SCKalimaCmd_QuaintonfbInfo, onQuaintonFbInfo)
	csmsgdispatcher.Reg(CrossSrvCmd.SCKalimaCmd, CrossSrvSubCmd.SCKalimaCmd_QuaintonInfo, onQuaintonInfo)
	...
	if data.anger >= QuaintonBossConfig[data.bossLv].maxAnger then
		engineevent.regGameStartEvent(openQuainton)
	end
	local fubenId = QuaintonFubenConfig[1].fbId
	insevent.registerInstanceEnter(fubenId, onEnterFb)
	insevent.registerInstanceEnterBefore(fubenId, onEnterBefore)
...
gmCmdHandlers.quaintonanger = function (actor, args)
	if System.isCommSrv() then
        SCTransferGM("quaintonanger", args)
    else
		local anger = tonumber(args[1])
		if not anger then end
		addAnger(anger)
    end
end
...
gmCmdHandlers.quaintonclose = function (actor, args)
	if System.isCommSrv() then
        SCTransferGM("quaintonclose", args)
    else
		closeQuainton()
    end
end
...
gmCmdHandlers.quaintoncreate = function (actor, args)
	if System.isCommSrv() then
        SCTransferGM("quaintoncreate", args)
    else
		local data = getGlobalData()
		data.anger = getMaxAnger()
		data.isOpen = 1
...
gmCmdHandlers.quaintonlevel = function (actor, args)
	if System.isCommSrv() then
        SCTransferGM("quaintonlevel", args)
    else
