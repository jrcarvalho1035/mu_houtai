...
local function getActorVar(actor)
    ...
    if not var.sjfb then
        var.sjfb = {
            maxPoint = getSJFBMaxPoint(actor)
        }
    end
    return var.sjfb
end
...
function changeSJPoint(actor, count, costType)
    ...
    local maxConfigPoint = getSJFBMaxPoint(actor)
...
function getSJFBMaxPoint(actor)
    if halosystem.isBuyHalo(actor) then
        return SJFBCommonConfig.maxPoint + SJFBCommonConfig.haloPoint
    end
    return SJFBCommonConfig.maxPoint
end
...
-------------------------------------------------------------------------------------------------------
--协议处理
--85-130 神迹秘境-请求秘境boss信息
local function c2sSJBossInfo(actor)
    s2cSJBossList(actor)
end

--85-130 神迹秘境-更新秘境boss信息
function s2cSJBossList(actor)
    ...
    LDataPack.writeShort(pack, #ShenJiBossConfig)
