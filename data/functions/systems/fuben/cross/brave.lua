...
function s2cBraveList(actor)
    ...
    if npack == nil then return end
    LDataPack.writeShort(npack, #BraveFubenConfig)
...
function c2sBraveBuy(actor, pactet)
    local var = getStaticData(actor)
    local lv = LActor.getSVipLevel(actor)
    if var.buycount >= SVipConfig[lv].brave then return end
...
local function onZhuansheng(actor, zslevel, oldLevel)
    local var = getStaticData(actor)
    for k, v in ipairs(BraveLevelConfig) do
        if zslevel >= v.zslevel then
            var.matchId = k
...
local function onEnterFb(ins, actor)
    ...
    instancesystem.s2cBelongData(actor, false, LActor.getActorById(bossData.belongId), bossData.hfuben) --归属者信息
...
local function onMonsterAiReset(ins, monster)
    ...
    bossData.nextShield = getNextShield(braveId)
    
    bossData.hpPercent = 100
    updateBraveBlood(braveId)
end
...
local function onActorDie(ins, actor, killHdl)
    ...
    local attacker_type = LActor.getEntityType(et)
    ...
    if LActor.getActorId(actor) == bossData.belongId then
        instancesystem.s2cBelongListClear(bossData.hfuben)
        --归属者被玩家打死，该玩家是新归属者
        if EntityType_Actor == attacker_type or EntityType_Role == attacker_type or EntityType_RoleSuper == attacker_type then
        ...
        elseif EntityType_Monster == attacker_type then --归属者被怪物打死，怪物无归属
            bossData.belongId = 0
        end
...
--跨服协议
---------------------------------------------------------------------------------
function sendBraveFbInfo(serverId)
    if not System.isBattleSrv() then return end
    local pack = LDataPack.allocPacket()
    LDataPack.writeByte(pack, CrossSrvCmd.SCBraveCmd)
    LDataPack.writeByte(pack, CrossSrvSubCmd.SCBraveCmd_SyncAllFbInfo)
    
    LDataPack.writeByte(pack, #BraveFubenConfig)
    for id, conf in ipairs(BraveFubenConfig) do
...
function onSendBraveFbInfo(sId, sType, dp)
...
function updateBraveFbInfo(id)
    ...
    local pack = LDataPack.allocPacket()
    LDataPack.writeByte(pack, CrossSrvCmd.SCBraveCmd)
    LDataPack.writeByte(pack, CrossSrvSubCmd.SCBraveCmd_SyncUpdateFbInfo)
    ...
    local name = data[id] and data[id].name or ""
    LDataPack.writeString(pack, name)
    
    System.sendPacketToAllGameClient(pack, 0)
end

function onUpdateBraveFbInfo(sId, sType, dp)
    if System.isCrossWarSrv() then return end
    local bossDatas = getBraveData()
    local id = LDataPack.readByte(dp)
    bossDatas[id] = {}
...
function updateBraveBlood(id)
    if not System.isBattleSrv() then return end
    local bossData = getBossData(id)
    if not bossData then return end
    local pack = LDataPack.allocPacket()
    LDataPack.writeByte(pack, CrossSrvCmd.SCBraveCmd)
    LDataPack.writeByte(pack, CrossSrvSubCmd.SCBraveCmd_SyncUpdateBlood)
...
function onUpdateBraveBlood(sId, sType, dp)
    if System.isCrossWarSrv() then return end
    local bossDatas = getBraveData()
    local id = LDataPack.readByte(dp)
...
function OnBraveConnected(serverId, serverType)
    if not System.isBattleSrv() then return end
    sendBraveFbInfo(serverId)
end

local function initGlobalData()
    if System.isLianFuSrv() then return end
    newsystem.regSystemOpenFuncs(actorexp.LimitTp.brave, onSystemOpen)
    --注册事件
    ...
    csbase.RegConnected(OnBraveConnected)
    
    csmsgdispatcher.Reg(CrossSrvCmd.SCBraveCmd, CrossSrvSubCmd.SCBraveCmd_SyncAllFbInfo, onSendBraveFbInfo)
    csmsgdispatcher.Reg(CrossSrvCmd.SCBraveCmd, CrossSrvSubCmd.SCBraveCmd_SyncUpdateFbInfo, onUpdateBraveFbInfo)
    csmsgdispatcher.Reg(CrossSrvCmd.SCBraveCmd, CrossSrvSubCmd.SCBraveCmd_SyncUpdateBlood, onUpdateBraveBlood)
...
gmCmdHandlers.flushAllBrave = function (actor, args)
    for id in ipairs(BraveFubenConfig) do
        local bossData = getBossData(id)
