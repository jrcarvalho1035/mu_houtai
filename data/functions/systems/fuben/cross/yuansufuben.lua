-- 元素幻境
module("yuansufuben", package.seeall)

YSBOSS_DATA = YSBOSS_DATA or {}
YSFUBEN_LIST = YSFUBEN_LIST or {}
YSBOSS_INDEX = YSBOSS_INDEX or {}

local function getActorVar(actor)
    if not actor then return end
    local var = LActor.getStaticVar(actor)
    if not var then return end
    if not var.ysfb then
        var.ysfb = {
            maxPoint = YSFBCommonConfig.maxPoint
        }
    end
    return var.ysfb
end

local function getYSBossData(id)
    return YSBOSS_DATA[id]
end

local function getConfigBybossId(bossId)
    local id = YSBOSS_INDEX[bossId]
    return YuanSuBossConfig[id]
end
...
local function getNextShield(id, hp)
    ...
    local conf = YuanSuBossConfig[id]
...
local function getYSBossPos(ins, bossId)
...
local function refreshBoss(_, id)
    local bossData = getYSBossData(id)
    local ins = instancesystem.getInsByHdl(bossData.hfuben)
    local handle = ins.scene_list[1]
    -- local scene = Fuben.getScenePtr(handle)
    print("yuansufuben refreshBoss handle =", ins.handle, " id =", id, "bossId =", bossData.bossId)
    local position = YuanSuBossConfig[id].refreshpos
...
function onEnerBossArea(ins, actor, bossId)
    ...
    local bossData = getYSBossData(config.id)
...
function YSBossFight(actor, ysId, floor)
    if not yuansusystem.checkYSOpen(actor, ysId) then return end
    
    local conf = YuanSuFubenConfig[ysId] and YuanSuFubenConfig[ysId][floor]
    if not conf then return end
    
    local var = getActorVar(actor)
    if var.maxPoint <= 0 then return end
    
    local hfuben = YSFUBEN_LIST[ysId] and YSFUBEN_LIST[ysId][floor]
    if not hfuben then return end
    
    local x, y = utils.getSceneEnterCoor(conf.fbId)
    local crossId = csbase.getCrossServerId()
    LActor.loginOtherServer(actor, crossId, hfuben, 0, x, y, 'yuansuboss')
end

function changeYSPoint(actor, count, costType)
    local var = getActorVar(actor)
    if not var then return end
    local beforePoint = var.maxPoint
    local afterPoint = beforePoint + count
    local maxConfigPoint = YSFBCommonConfig.maxPoint
    if halosystem.isBuyHalo(actor) then
        maxConfigPoint = maxConfigPoint + YSFBCommonConfig.haloPoint
    end
    if afterPoint <= 0 then
        afterPoint = 0
        LActor.postScriptEventLite(actor, 3 * 1000, exitYSFb)
    ...
    utils.logCounter(actor, 'changeYSPoint', count, extra, costType)
end

function exitYSFb(actor)
...
local function onEnterBefore(ins, actor)
    slim.s2cMonsterConfig(actor, {YSFBCommonConfig.tombMonId})
end

local function onEnterFb(ins, actor)
    LActor.setCamp(actor, CampType_Normal)
    local count = YSFBCommonConfig.consumePoint
    LActor.postScriptEventEx(actor, YSFBCommonConfig.consumeTime * 1000, function(...) changeYSPoint(actor, -count) end, YSFBCommonConfig.consumeTime * 1000, -1)
end
...
local function onMonsterAiReset(ins, monster)
    ...
    local config = getConfigBybossId(bossId)
    if config == nil then
        print('yuansufuben.onMonsterAiReset config==nil bossId=' .. bossId)
        return
    end
    local index = config.id
    local bossData = getYSBossData(index)
...
local function onBossDie(ins, monster, killHdl)
    ...
    print("yuansufuben onBossDie handle =", ins.handle, " id =", bossData.id, "bossId =", bossId)
    ...
    local rewardPoint = YSFBCommonConfig.rewardPoint
        ...
        if actorId == bossData.belongId then
            ...
            if var.maxPoint > 0 then
                ...
                if var.isDouble == 1 then
                    ...
                    changeYSPoint(belong, -(rewardPoint * 2), "belongReward double")
                else
                    changeYSPoint(belong, -rewardPoint, "belongReward")
                end
                sendYSRewardResult(belong, 1, belongInfo, rewards, serverId)
                s2cYSMaxPoint(belong)
            end
        else
            ...
            if actor then
                ...
                if var.maxPoint > 0 then
                    ...
                    if var.isDouble == 1 then
                        ...
                        changeYSPoint(belong, -(rewardPoint * 2), "joinReward")
                    else
                        changeYSPoint(actor, -rewardPoint, "joinReward")
                    end
                    sendYSRewardResult(actor, 0, belongInfo, rewards, serverId)
                    s2cYSMaxPoint(actor)
                end
            end
        end
    ...
    -- 更新副本玩家
    broadYSBossInfo(ins.handle, index, refresh_endtime)
    -- 更新普通服
    updateYSFbInfo(index)
    -- 创建boss墓碑
    local tomb = Fuben.createMonster(scene_hdl, YSFBCommonConfig.tombMonId, x, y, refreshtime, 0, bName)
    if tomb then
...
-------------------------------------------------------------------------------------------------------
--协议处理
--85-80 元素幻境-请求元素boss信息
local function c2sYSBossInfo(actor)
    s2cYSBossList(actor)
end

--85-80 元素幻境-更新元素boss信息
function s2cYSBossList(actor)
    if next(YSBOSS_DATA) == nil then return end
    
    local var = getActorVar(actor)
    if not var then return end
    
    local pack = LDataPack.allocPacket(actor, Protocol.CMD_Cross, Protocol.sCsYSBoss_Info)
    if pack == nil then return end
    
    LDataPack.writeInt(pack, var.maxPoint)
    LDataPack.writeChar(pack, #YuanSuBossConfig)
    for id, conf in pairs(YuanSuBossConfig) do
        local bossData = getYSBossData(id)
...
--85-81 元素幻境-挑战元素boss
local function c2sYSBossFight(actor, pack)
    local ysId = LDataPack.readChar(pack)
    ...
    YSBossFight(actor, ysId, floor)
end

--85-82 元素幻境-副本内更新元素boss复活状态
function broadYSBossInfo(fbhdl, id, refreshtime)
    local pack = LDataPack.allocPacket()
    LDataPack.writeByte(pack, Protocol.CMD_Cross)
    LDataPack.writeByte(pack, Protocol.sCsYSBoss_BossInfo)
    LDataPack.writeChar(pack, id)
    LDataPack.writeInt(pack, refreshtime)
    Fuben.sendData(fbhdl, pack)
end

--85-82 元素幻境-副本外更新元素boss复活状态
function broadYSFbInfo(id)
    local bossData = getYSBossData(id)
...
--85-83 元素幻境-玩家战斗结算
function sendYSRewardResult(actor, res, belongInfo, rewards, serverId)
    if actor and actoritem.checkEquipBagSpaceJob(actor, rewards) then
        actoritem.addItems(actor, rewards, "ysfb rewards")
    else
        local mailData = {head = YSFBCommonConfig.mailTitle, context = YSFBCommonConfig.mailContent, tAwardList = rewards}
        mailsystem.sendMailById(LActor.getActorId(actor), mailData, serverId)
    end
    
    local pack = LDataPack.allocPacket(actor, Protocol.CMD_Cross, Protocol.sCsYSBoss_Result)
    if pack == nil then return end
...
--85-84 元素幻境-更新元素能量值
function s2cYSMaxPoint(actor)
    local var = getActorVar(actor)
    if not var then return end
    
    local pack = LDataPack.allocPacket(actor, Protocol.CMD_Cross, Protocol.sCsYSBoss_MaxPoint)
    if pack == nil then return end
...
--85-85 元素幻境-设置双倍消耗状态
local function c2sYSBossSetDouble(actor)
...
--85-85 元素幻境-更新双倍消耗状态
function s2cYSBossSetDouble(actor)
    local var = getActorVar(actor)
    if not var then return end
    
    local pack = LDataPack.allocPacket(actor, Protocol.CMD_Cross, Protocol.sCsYSBoss_SetDouble)
    if pack == nil then return end
...
--跨服协议
--跨服向普通服同步boss信息
function sendYSFbInfo(serverId)
    if not System.isBattleSrv() then return end
    local pack = LDataPack.allocPacket()
    LDataPack.writeByte(pack, CrossSrvCmd.SCYuanSuCmd)
    LDataPack.writeByte(pack, CrossSrvSubCmd.SCYuanSuCmd_SyncAllFbInfo)
    
    LDataPack.writeByte(pack, #YuanSuFubenConfig)
    for ysId, config in ipairs(YuanSuFubenConfig) do
        LDataPack.writeByte(pack, #config)
        for floor, conf in ipairs(config) do
            LDataPack.writeInt64(pack, YSFUBEN_LIST[ysId][floor])
        end
    end
    
    LDataPack.writeByte(pack, #YuanSuBossConfig)
    for id in ipairs(YuanSuBossConfig) do
        local bossData = getYSBossData(id)
...
--普通服收到跨服boss信息
function onSendYSFbInfo(sId, sType, dp)
    if System.isCrossWarSrv() then return end
    ...
    for ysId = 1, count do
        ...
        for floor = 1, cnt do
            YSFUBEN_LIST[ysId][floor] = LDataPack.readInt64(dp)
        end
    end
    ...
    for i = 1, number do
        ...
        YSBOSS_DATA[id] = {}
        YSBOSS_DATA[id].hfuben = LDataPack.readInt64(dp)
        YSBOSS_DATA[id].bossId = LDataPack.readInt(dp)
        YSBOSS_DATA[id].refreshtime = LDataPack.readInt(dp)
    end
end

--跨服给普通服更新单个boss信息
function updateYSFbInfo(id)
    if not System.isBattleSrv() then return end
    local bossData = getYSBossData(id)
    if not bossData then return end
    ...
    local pack = LDataPack.allocPacket()
    LDataPack.writeByte(pack, CrossSrvCmd.SCYuanSuCmd)
    LDataPack.writeByte(pack, CrossSrvSubCmd.SCYuanSuCmd_SyncUpdateFbInfo)
...
--普通服收到更新单个boss信息
function onUpdateYSFbInfo(sId, sType, dp)
    if System.isCrossWarSrv() then return end
    local id = LDataPack.readByte(dp)
    YSBOSS_DATA[id] = {}
    YSBOSS_DATA[id].hfuben = LDataPack.readInt64(dp)
...
--连接跨服事件
local function onYSFBConnected(serverId, serverType)
    sendYSFbInfo()
end
----------------------------------------------------------------------------------
--初始化
local function initGlobalData()
    ...
    netmsgdispatcher.reg(Protocol.CMD_Cross, Protocol.cCsYSBoss_Info, c2sYSBossInfo)
    netmsgdispatcher.reg(Protocol.CMD_Cross, Protocol.cCsYSBoss_Fight, c2sYSBossFight)
    netmsgdispatcher.reg(Protocol.CMD_Cross, Protocol.cCsYSBoss_SetDouble, c2sYSBossSetDouble)
    
    csmsgdispatcher.Reg(CrossSrvCmd.SCYuanSuCmd, CrossSrvSubCmd.SCYuanSuCmd_SyncAllFbInfo, onSendYSFbInfo)
    csmsgdispatcher.Reg(CrossSrvCmd.SCYuanSuCmd, CrossSrvSubCmd.SCYuanSuCmd_SyncUpdateFbInfo, onUpdateYSFbInfo)
    
    if not System.isBattleSrv() then return end
    for _, config in pairs(YuanSuFubenConfig) do
        for __, conf in ipairs(config) do
            ...
            YSFUBEN_LIST[conf.ysId] = YSFUBEN_LIST[conf.ysId] or {}
            if not YSFUBEN_LIST[conf.ysId][conf.floor] then
                YSFUBEN_LIST[conf.ysId][conf.floor] = instancesystem.createFuBen(conf.fbId)
            end
        end
    end
    
    for id, conf in ipairs(YuanSuBossConfig) do
        local hfuben = YSFUBEN_LIST[conf.ysId][conf.floor]
        if not YSBOSS_DATA[id] then
            YSBOSS_DATA[id] = {
                id = id,
                hpPercent = 100,
                hfuben = hfuben,
        ...
        YSBOSS_INDEX[conf.bossId] = id
    end
    csbase.RegConnected(onYSFBConnected)
end
...
gmCmdHandlers.YSreset = function (actor, args)
    local var = getActorVar(actor)
    var.maxPoint = YSFBCommonConfig.maxPoint
    return true
end

gmCmdHandlers.YSPrint = function (actor, args)
    local var = getActorVar(actor)
    print("maxPoint =", var.maxPoint)
    return true
end

gmCmdHandlers.YSFight = function (actor, args)
    local ysId = tonumber(args[1])
    local floor = tonumber(args[2])
    YSBossFight(actor, ysId, floor)
    return true
end

gmCmdHandlers.YSFbPrint = function (actor, args)
    utils.printTable(YSBOSS_DATA)
    utils.printTable(YSFUBEN_LIST)
    utils.printTable(YSBOSS_INDEX)
    return true
end
