...
function checkScriptTimer()
	...
	for pos, info in ipairs(_G.SCRIPTTIMELIST) do
		...
		if info.exeTime then
			...
			--与开服时间不相关的,进行一系列检查
			if isExe and conf.year and checkTime(conf.year, year) == false then isExe = false end --新增了对年的处理
...
function reg(timeInfo)
	...
	assert(type(_G[timeInfo.func]) == "function")
	if timeInfo.params then
		assert(type(timeInfo.params) == "table")
	end
	table.insert(_G.SCRIPTTIMELIST, {config = timeInfo})
end

table.insert(InitFnTable, initScriptTimer)
