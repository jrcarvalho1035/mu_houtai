...
function cleanActorList(actor)
    ...
    print("postscripttimer.cleanActorList:clear idx:"..idx)
...
function onTimeEvent(actor, eid)
	...
	if event.func then
		xpcall(function() event.func(actor, unpack(event.arg)) end, script_error_handle)
	end
...
function cancelScriptTimer(entity, eid)
    print("postscripttimer.cancelScriptTimer:clear eid:"..eid)
	if not entity then
		entity = System.getGlobalNpc()
	end
